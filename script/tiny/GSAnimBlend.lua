...
local blendCommand = [[getmetatable(_ENV).GSLib_triggerBlendTiny(%q)]]

_ENVMT.GSLib_triggerBlendTiny = setmetatable({}, {
  ...
  for anim in pairs(blending) do
    ...
    if not state.paused then
      ...
      if state.time > state.max or (animGetPlayState(anim) == "STOPPED") then
        ...
        animBlend(from + (state.to - from) * this.curve(state.time / state.max))
      end
    end
  end
  last_delta = delta
